var movies = ["The Matrix", "The Notebook", "Mr. Nobody", "The Lion King", "Avenger","Star Wars","Hostel","300","Creed","Fight Club", "Tangled", "Night out"];
...
for(let i = 0 ; i<movies.length; i++){
  ...
  $.ajax({
   ...
   row.append("<a href='/detail' id='" + response.imdbID + "' class='btn btn-info more-detail'>"  + "More Detail" + "</a>");
...
function getMovie(){
  ...
  $.ajax({
   ...
   $(".img-box").append("<img src=" + response.Poster + " class='img-fluid'/>");
   var row = $("<div>");
   ...
   row.append("<p> <strong> IMDB Rating: </strong>" + response.imdbRating + "</p>");
   ...
   row.append("<a href='"+ response.Website +"' class='btn btn-info' target='_blank' > Wartch Now </a>");
...
$(document).on("click", ".search-movie" , function() {
  var movieSearch=document.getElementById("search-input").value;
